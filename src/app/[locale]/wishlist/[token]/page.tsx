...
export default function WishlistSharePage() {
  const t = useTranslations('wishlistShare');
    ...
    async function fetchWishlist() {
      try {
        ...
        if (!response.ok) {
          ...
          const errorMessage = errorData.details || errorData.error || t('loadingError');
        ...
        if (data.owner) {
          const name = data.owner.full_name || data.owner.username || t('defaultUserName');
          setOwnerName(name);
        }
      } catch (error) {
        console.error('Error fetching wishlist:', error);
        toast.error(t('wishlistLoadingError'));
      } finally {
        setLoading(false);
      }
    }
              ...
              <div className="mb-6 md:mb-8">
                ...
                {ownerName && (
                  <p className="text-lg text-muted-foreground mb-2">
                    {t('ownerPrefix')} <span className="font-semibold text-foreground">{ownerName}</span>
                  </p>
                )}
