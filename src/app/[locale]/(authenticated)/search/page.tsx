    ...
    const fetchPlatformsFromSupabase = async () => {
      try {
        ...
        const allPlatforms: Platform[] = [
          { id: 'all', name: t('allPlatforms'), igdb_platform_id: 0, abbreviation: 'ALL' },
          ...formattedPlatforms
        ];
        ...
        toast.error(t('errorLoadingPlatforms'));
      }
    };
    ...
    try {
      ...
      if (existingGame) {
        ...
        if (existingUserGame) {
          ...
          toast.error(
            t('gameAlreadyInCollection', { game: selectedGame.name, console: consoleDisplayName }), 
            { 
              duration: 4000,
              icon: '🚫',
              style: {
      ...
      if (isNewConsole) {
        ...
        toast.success(
          t('gameAddedForConsole', { game: selectedGame.name, console: consoleDisplayName }),
          { 
            duration: 4000,
            icon: '🎉',
            style: {
        ...
        // Nouveau jeu ajouté
        toast.success(
          t('gameAddedSuccess', { game: selectedGame.name }),
          { 
            duration: 4000,
            icon: '🎉',
            style: {
      ...
      toast.error(error.message || t('errorAddingGame'));
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-border/50 p-6 md:p-8 shadow-xl">
        ...
        <div className="relative space-y-4">
          ...
          <p className="text-xs text-muted-foreground italic"><span className="font-bold">{t('warningTitle')}</span> {t('warningText')}</p>
        </div>
        </div>
      </section>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-base font-bold">
                {t('selectPlatform')} {selectedPlatform !== null && filteredPlatforms.find(p => p.igdb_platform_id === selectedPlatform)?.name ? `(${filteredPlatforms.find(p => p.igdb_platform_id === selectedPlatform)?.name})` : `(${tCommon('optional')})`}
              </h2>
            </div>
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <span className="font-medium">{t('allPlatforms')}</span>
                      </div>
                    </div>
      ...
      {selectedPlatform && selectedPlatform > 0 && (
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border border-primary/20 p-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-muted-foreground">{t('platformSelected')}</span>
      ...
      {isError ? (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-destructive/10 via-card to-card/95 border border-destructive/20 shadow-xl backdrop-blur-sm p-12">
          ...
          <div className="relative flex flex-col items-center justify-center gap-4 text-center">
            <p className="text-destructive font-medium text-lg">{t('errorOccurred')}</p>
            <Button 
              onClick={() => {
                setShouldSearch(true);
                refetchRef.current();
        ...
        <>
              ...
              <div className="relative flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-muted-foreground text-lg">
                  {selectedPlatform !== null && selectedPlatform > 0 
                    ? t('selectGameOn', { platform: platforms.find(p => p.igdb_platform_id === selectedPlatform)?.name || 'la plateforme' })
                    : t('typeGameName')}
                </p>
                ...
                {selectedConsoleName && (
                  <div className="mt-2 rounded-lg bg-primary/20 border border-primary/30 p-3">
                    <p className="text-sm text-muted-foreground">
                      {t('lastConsoleSelected')} <span className="font-semibold text-primary">{selectedConsoleName}</span>
                    </p>
                  </div>
                )}
              </div>
                      ...
                      <div className="relative aspect-[3/4] overflow-hidden rounded-t-lg">
                        ...
                        {game.platforms && game.platforms.length > 0 && (
                          <div className="absolute top-2 right-2 bg-black/70 text-white px-1.5 py-0.5 rounded text-xs">
                            {game.platforms.length} {game.platforms.length > 1 ? t('platforms') : t('platform')}
                          </div>
                        )}
                      </div>
                      <div className="p-2">
          ...
          {searchQuery && data?.pages && data.pages[0] && data.pages[0].games && data.pages[0].games.length === 0 && !isLoading && (
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-card via-card to-card/95 border border-border/50 shadow-xl backdrop-blur-sm p-12">
              ...
              <div className="relative flex flex-col items-center justify-center gap-4 text-center">
                <Search className="h-12 w-12 text-muted-foreground/50" />
                <p className="text-muted-foreground text-lg">{t('noResultsFound')}</p>
                <p className="text-sm text-muted-foreground/70">{t('tryOtherKeywords')}</p>
              </div>
            </div>
          )}
        </>
