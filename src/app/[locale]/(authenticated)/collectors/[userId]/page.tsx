...
function CollectorCollectionContent() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        // Afficher le toast seulement une fois par utilisateur
        if (!hasShownErrorToastRef.current) {
          toast.error(t('profileLoadingError'));
  ...
  useEffect(() => {
    ...
    if (error && isProfilePublic === true && error.message !== 'Ce profil est privé') {
      toast.error(error.message || t('collectionLoadingError'));
    }
  ...
  const { consoles, genres } = useMemo(() => {
    if (!statsData) {
      return {
        consoles: [{ id: 'all', name: t('all') + ' ' + t('platforms').toLowerCase(), count: 0 }],
        genres: [{ id: 'all', name: t('all') + ' ' + t('genres').toLowerCase(), count: 0 }]
      };
    }
    ...
    const consoleList = [
      { id: 'all', name: t('all') + ' ' + t('platforms').toLowerCase(), count: statsData.totalGames },
      ...sortedConsoles
    ];

    const genreList = [
      { id: 'all', name: t('all') + ' ' + t('genres').toLowerCase(), count: statsData.totalGames },
      ...sortedGenres
    ];
          ...
          <>
            ...
            <div className="relative mb-4 md:hidden">
              ...
              >
                ...
                <span className="text-sm font-medium">{t('backToCollectors')}</span>
              </Button>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-foreground via-foreground to-foreground/70 bg-clip-text text-transparent truncate">
                      {t('collectionOf', { username: profile?.username || t('collector') })}
                    </h1>
                    <p className="text-xs sm:text-sm md:text-base text-muted-foreground mt-1">
                      {t('gamesInCollection', { count: statsData?.totalGames || 0 })}
                    </p>
                  </div>
                </div>
              </div>
