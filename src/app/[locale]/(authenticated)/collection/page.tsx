...
function CollectionPageContent() {
  ...
  const handleGetShareLink = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(t('linkGenerationError'));
      }
      ...
      if (data.error) {
        ...
        toast.error(data.details || data.error || t('linkGenerationError'));
        return;
      }
      ...
      toast.error(error.message || t('linkGenerationError'));
    } finally {
  ...
  const handleToggleShare = async () => {
    try {
      ...
      toast.success(data.message || (newStatus ? t('shareActivated') : t('shareDeactivated')));
    } catch (error: any) {
      console.error('Error toggling share:', error);
      toast.error(error.message || t('shareError'));
    }
  };
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-foreground via-foreground to-foreground/70 bg-clip-text text-transparent">
              {activeTab === 'wishlist' ? t('myWishlist') : t('myCollection')}
            </h1>
            <p className="text-sm sm:text-base text-muted-foreground">
              {activeTab === 'wishlist'
                ? t('wishlistDescription', { count: totalGames })
                : t('collectionDescription', { count: totalGames })
              }
            </p>
          </div>
              ...
              {isLoadingShare ? (
                ...
                <>
                  <Button
                    ...
                    title={isShareActive ? t('shareDeactivated') : t('shareActivated')}
                  >
                    <Power className={`h-4 w-4 ${isShareActive ? 'text-primary' : 'text-muted-foreground'}`} />
                    <span className="hidden sm:inline text-sm">{isShareActive ? t('sharingActive') : t('sharingDisabled')}</span>
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    ...
                    className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3"
                    title={!isShareActive ? t('activateShare') : t('copyShareLink')}
                  >
                    {copied ? (
                      <>
                        <Check className="h-4 w-4" />
                        <span className="hidden sm:inline text-sm">{t('copied')}</span>
                      </>
                    ) : (
                      <>
