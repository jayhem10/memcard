...
export async function POST(_request: NextRequest) {
  ...
  try {
    ...
    if (gamesError) return NextResponse.json({ success: false, message: gamesError.message }, { status: 500 });
    if (pricesError) return NextResponse.json({ success: false, message: pricesError.message }, { status: 500 });
    ...
    if (missing.length) {
      ...
      if (insertMissingError) {
        return NextResponse.json({ success: false, message: insertMissingError.message }, { status: 500 });
      }
      // Rafraîchir la map
    ...
    return NextResponse.json({ success: true, message, updated, skipped, errorsCount: errors.length, total, errorSamples });
  } catch (e: any) {
    return NextResponse.json({ success: false, message: e?.message || 'Erreur interne' }, { status: 500 });
  }
}
